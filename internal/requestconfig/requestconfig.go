...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("Miru/Go %s", internal.PackageVersion),
	}
}
...
func SubstituteServerVariables(templateURL *url.URL, cfg *RequestConfig) (*url.URL, error) {
	...
	if strings.Count(baseURL, "HOST") >= 1 {
		if cfg.Host == "" {
			return nil, fmt.Errorf("must provide Host to substitute %s", baseURL)
		}
		baseURL = strings.ReplaceAll(baseURL, "HOST", cfg.Host)
	}
	if strings.Count(baseURL, "VERSION") >= 1 {
		if cfg.Version == "" {
			return nil, fmt.Errorf("must provide Version to substitute %s", baseURL)
		}
		baseURL = strings.ReplaceAll(baseURL, "VERSION", cfg.Version)
	}
